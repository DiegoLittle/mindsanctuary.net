...
export default function MoodsCalendar({moodLogData,setMoodLogData}) {
  // const {moodLogData,setMoodLogData} = useContext(moodLogContext)
  const [yearMonth,setYearMonth] = useState(dayjs().format('YYYY-MM'))
  const [days, setDays] = useState([])
  const [moods, setMoods] = useState([])
  // console.log("calendar",moodLogData)
  interface Day {
    date:string
    mood:{}
  }
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      days.forEach((day, index) => {
        ...
        //   if(moodLogData.moodLogData.moodvalue!=null){
        //   days[index].mood = moodLogData.moodLogData.moodvalue
        // }
        // }
          ...
          <button
            ...
            className={classNames(
              ...
              dayjs(day.date).month() == dayjs(yearMonth).month()
                ? 'bg-white'
                : 'bg-gray-50',
              dayjs(moodLogData.createdAt).format("YYYY-MM-DD")===day.date
